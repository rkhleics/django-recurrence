...
function initRecurrenceWidget($field) {
    if (!$field) {
        const recurrenceFields = $(document).find('textarea.recurrence-widget:not([id*="__prefix__"])');
        $.each(recurrenceFields, function(index, field) {
            const $field = $(field);
    ...
    for (const mutation of mutationsList) {
        if (mutation.type === 'childList') {
            ...
            if (addedNodes.length > 0) {
                const $addedRecurrenceField = $(addedNodes[0]).find('.recurrence-widget');
